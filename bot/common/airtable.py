...
async def get_contribution_records(guild_id):
    ...
    return list_contributions(guild_id)


# TODO can we elminate
async def get_highest_contribution_records(guild_id, user_id, total):
    ...
    # return await loop.run_in_executor(None, _get_contribution)
    logger.waring("This query has not been converted yet")
    return
...
async def get_contribution_count(user_id, guild_id):
    ...
    return await get_contributuion_count(user_id, guild_id)
...
async def create_user(user_id, guild_id):
    ...
    #         discord_record = find_user(user_id)
    #         if discord_record == "":
    ...
    user = create_user()
    return await create_guild_user(user_id, guild_id)
